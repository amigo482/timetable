...
import { Row, Input, option, Icon, Button, Badge, Col } from 'react-materialize';
...
class Elector extends React.Component {
  ...
  changePlusMinus() {
    if (this.state.plus_minus === 'disabled') {
      ...
      this.props.clearFields(false, false, 'plus_minus');
    }
  }
  ...
  createInput(props) {
    const { type, title, label, disabled, data, input } = props;
  ...
  render() {
    const { teachers, subjects, auditories, handleSubmit, saveCouple } = this.props;
